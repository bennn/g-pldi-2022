...
(define (make-huffman-table size-counts values)
  (let* ((count (bytes-length values))
         (size-offsets : (Vectorof (U #f Natural)) (make-vector 16 #f))
         (sizes (make-bytes count 0))
         (codes : (Vectorof (U #f Natural)) (make-vector count #f))
         (value-indexes : (Vectorof (U #f Natural)) (make-vector 256 #f))
         (max-codes : (Vectorof Integer) (make-vector 16 -1)))
    ;; A reverse map from value to index.
    (for ((i : Natural (in-naturals))
          (value values))
      (vector-set! value-indexes value i))
    ;; Compute sizes for each value.
    (let lp ((size : Natural 0) (offset : Natural 0))
      (when (< size 16)
...
(: print-huffman-table (-> Huffman Void))
...
(define (read-huffman-coded-value bit-port table)
  ...
  (match table
    ((vector size-counts size-offsets
             values value-indexes sizes codes max-codes)
     (let lp ((size-idx : Natural 0) (code : Integer (read-bit bit-port)))
       (cond
...
(define (compute-huffman-code-sizes-for-freqs freqs)
  (let ((sizes (make-bytes 257 0))
    ...
    (define (find-least-idx)
      (let lp : Natural ((i : Natural 0) (least-idx : (U #f Natural) #f))
        (if (< i 257)
            (lp (add1 i)
    ...
    (define (find-next-least least-idx)
      (let lp : (U #f Natural) ((i : Natural 0) (next-least-idx : (U #f Natural) #f))
        (if (< i 257)
            (lp (add1 i)
                (let ((x (assert (vector-ref freqs i) values)))
  ...
  (call-with-values (lambda ()
    ...
    (lambda ((sizes : Bytes) (max-size : Natural))
      (let ((size-counts (make-bytes (max max-size 16) 0)))
        ...
        (let adjust-bits : Void ((i : Natural (assert (sub1 max-size) natural?)))
          (cond
           ...
           (else
            ...
            (let lp : Void ((j : Natural (assert (- i 2) natural?)))
              (cond
        ...
        (let* ((count (for/fold : Natural ((sum : Natural 0)) ((count : Natural size-counts))
                        (+ sum count)))
               (values (make-bytes count 0)))
          (let visit-size : Void ((size : Natural 1) (k : Natural 0))
            (when (<= size max-size)
              (let visit-values : Void ((j : Natural 0) (k : Natural k))
                (cond
                 ((= j 256)
