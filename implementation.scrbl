...
semantics) [CITE] with the @|sShallow| Racket implementation of @|stransient|@~cite{glfd-pj-2021}.
...
Changing from one to the other is a one-line change except in
programs that manipulate type boundaries directly.
...
@; TODO LAL maybe good example of complexity ... if we're too simple, forget these details

@section[#:tag "sec:implementation:ctc-indirect"]{Separate Compilation, Lazy Loading, Type Lookup}
;; ctc indirection

TR wants separate comp
wants to not apply / make contracts unless needed
laziness => 2 identifiers
the split = harder to lookup types ... can only resolve statically


@section[#:tag "sec:implementation:ctc-gen"]{Annotation Burden, Contract Generation}
@; TODO debug diary, use optional to introduce
@; ... maybe another diary for prefabs

franly i don't havae a story yet


@section[#:tag "sec:implementation:macro"]{Macros and Hidden Exports}
@; hidden exports ... unregulated exports

old challenge from LAL? from POPL?


@section[#:tag "sec:implementation:api"]{Logical Types, Type-Boundary API}
@; TB api
